...
КАРТА КОНКУРЕНТОВ ПО ГОРОДАМ АЯНА (актуально 1 кв. 2026, проверено через 2ГИС):

КАРАГАНДА — всего ~70 точек у конкурентов:
- Fix Price: 25 точек (главный по средним супермаркетам/дискаунтерам)
- Южный: 13 точек (крупный супермаркет, региональный лидер)
- Nan хлеб: 12 точек (магазины у дома, хлебная сеть)
- Magnum: 5 точек (Бухар-Жырау 86 — GrandStore, Восток-1, Шахтёров 82, 137-й уч. кв., 134-й уч. кв.)
- Optima: 5 точек (Абдирова, Анжерская, Ермекова, Зелинского, 12-й мкр)
- Корзина: 4 точки (Степной-3, Язева, Мамыраева, 22-й мкр)
- Светофор: 4 точки (Молокова 117Б, Складская 2а, Старогорняцкая, 18-й мкр)
- ЮМА: 2 точки, Norman: 2, Real: 2, Ежик: 2, Доброцен: 1, Guljan: 1
- Small: НЕТ в Караганде по реестру 1 кв. 2026 (был ранее, фактически закрылся)
- Galmart: НЕТ
- Arbuz.kz: НЕТ

АСТАНА — крупнейший конкурентный рынок:
- Magnum: 78 точек (форматы Daily, Express, Magnum) — абсолютный лидер
- Fix Price: 67 точек
- SMALL: 63 точки
- My Mart: 41 точка (мини-маркеты у дома)
- Istop: 32 точки (мини-маркеты на остановках, без алкоголя)
- Apero: 30 точек (мини-маркеты у дома)
- Вкусмарт: 10 точек
- Сердце столицы: 9 (мини-маркеты), Ali Mart: 9 (мини-маркеты)
- Galmart: 8 точек (Керуен, MEGA Silk Way, Кабанбай Батыра 62 и др.) — премиум-сегмент
- OrdaMarket: 5 точек
- Анвар: 4, Spar (SMALL): 6, Astor: 2, Greenwich: 2, Interfood: 2
- Южный (Гастроном): 2 точки (Сарайшык, Абикен Бектуров)
- Arbuz.kz: онлайн-сервис доставки (приложение arbuz.kz и Freedom Bank SuperApp), физических магазинов нет

ТЕМИРТАУ — конкурентов мало, главный рынок для Аяна:
- Fix Price: 13 точек (главный конкурент)
- Гастроном/Металлург: 3 точки (Республики 27, Металлургов 13, Абая 104)
- Корзина: 1 точка (Металлургов 33)
- Южный: 1 точка (мкр 3а, 5)
- Светофор: 1 точка (мкр 9, 3м)
- Magnum: НЕТ в Темиртау
- Small: НЕТ в Темиртау
- Galmart: НЕТ

ВЫВОДЫ ДЛЯ СТРАТЕГИИ:
- В Темиртау Аян — практически монополист в премиум/средний сегменте, главный конкурент только Fix Price (дискаунтер)
- В Караганде главное противостояние с Южным (13 точек) и Magnum (5 крупных)
- В Астане Аян — нишевый игрок против Magnum (78), Galmart (премиум), Small (63). Стратегия: дифференциация через эксклюзивные трендовые SKU
- Fix Price везде сильный — но это дискаунтер с фиксированной ценой, премиум-тренды туда не зайдут — это окно для Аяна

При упоминании конкурентов используй ТОЛЬКО реальные города их присутствия из карты выше.
              ...
              >
                ...
                {showUpdateTooltip && (
                  <div style={{position:"absolute",top:"calc(100% + 8px)",left:0,zIndex:9999,background:"#0f172a",border:"1px solid #334155",borderRadius:10,padding:"14px 16px",minWidth:340,boxShadow:"0 8px 32px rgba(0,0,0,0.25)"}}>
                    ...
                    {allUpdates.length === 0 ? (
                      <div style={{fontSize:11,color:"#64748b",fontStyle:"italic"}}>Обновлений пока нет</div>
                    ) : (
                    ...
                    <div style={{marginTop:10,fontSize:10,color:"#475569",fontStyle:"italic"}}>Категории без обновлений не отображаются</div>
                  </div>
                )}
              </div>
            );
