...
const sb = {
  ...
  async getAssessment(category, city) {
    try {
      const cityQ = city ? `&city=eq.${encodeURIComponent(city)}` : "";
      const r = await fetch(`${SUPABASE_URL}/rest/v1/assortment_assessment?category=eq.${encodeURIComponent(category)}${cityQ}&order=created_at.desc&limit=1&select=*`, {
        headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` }
      });
      if (!r.ok) return null;
      const data = await r.json();
  ...
  async saveAssessment(category, city, sessionId, trendsCount, assessment) {
    try {
      // Удаляем старую оценку этой категории+города — храним только свежую
      const cityQ = city ? `&city=eq.${encodeURIComponent(city)}` : "";
      await fetch(`${SUPABASE_URL}/rest/v1/assortment_assessment?category=eq.${encodeURIComponent(category)}${cityQ}`, {
        method: "DELETE",
        headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` }
      });
...
const CATEGORIES = ["Все","Снеки","Напитки","Молочка","Здоровое питание","Бытовая химия","Кондитерка","Готовая еда","Мороженое","Полуфабрикаты","Морепродукты","Детское питание","Мама и младенец","Колбасные изделия","Соусы","Овощи и фрукты","Хлебобулочные","Алкоголь","Высокобелковые","Консервация","Free From / Без..."];
...
const CAT_ICONS = {"Мороженое":"🍦","Полуфабрикаты":"🥩","Морепродукты":"🦐","Детское питание":"🍼","Мама и младенец":"👶","Колбасные изделия":"🌭","Соусы":"🫙","Овощи и фрукты":"🥦","Хлебобулочные":"🍞","Алкоголь":"🍺","Высокобелковые":"💪","Здоровое питание":"🌿","Консервация":"🥫","Free From / Без...":"🚫"};
...
const CAT_DESCRIPTIONS = {
  ...
  "Free From / Без...":"Продукты без определённых ингредиентов: безлактозные (Valio, Arla), растительное молоко (Oatly, Alpro), без сахара, без глютена. Для людей с непереносимостью и тех кто выбирает осознанно.",
};
...
function AssortmentModal({ assortment, filter, trends, onClose, onUpload, assortmentLoading, onCleanup, cleaningAssortment }) {
  ...
  // Города, доступные в данных этой категории
  const catScope = isAll ? assortment : assortment.filter(a => a.category === filter);
  const citiesAvailable = [...new Set(catScope.map(a => a.city).filter(Boolean))].sort();
  const hasCities = citiesAvailable.length > 0;
  const [cityFilter, setCityFilter] = useState("Все");

  // Если выбранный город пропал из данных (сменили категорию) — сбрасываем
  useEffect(() => {
    if (cityFilter !== "Все" && !citiesAvailable.includes(cityFilter)) setCityFilter("Все");
    // eslint-disable-next-line
  }, [filter, citiesAvailable.join(",")]);

  // Для режима конкретной категории — фильтр по категории + городу
  const catData = (isAll ? assortment : assortment.filter(a => a.category === filter))
    .filter(a => cityFilter === "Все" || a.city === cityFilter);

  // Группируем по сессиям.
  // При выборе конкретного города — обычная история сессий этого города.
  // При "Все города" — каждый город имеет свою сессию с одной датой, поэтому
  // объединяем последние сессии всех городов в одну виртуальную, чтобы не считать
  // города "периодами" в блоке Динамики.
  let sessionList, latestSession, firstSession, hasHistory;
  if (cityFilter === "Все" && hasCities && !isAll) {
    // Собираем items последней сессии каждого города
    const latestSidByCity = {};
    catData.forEach(item => {
      const c = item.city || "—";
      const d = new Date(item.session_date || item.uploaded_at || 0).getTime();
      if (!latestSidByCity[c] || d > latestSidByCity[c].ts) {
        latestSidByCity[c] = { sid: item.session_id, ts: d, date: item.session_date || item.uploaded_at, period: item.period };
      }
    });
    const mergedItems = catData.filter(item => {
      const c = item.city || "—";
      return latestSidByCity[c] && item.session_id === latestSidByCity[c].sid;
    });
    const anyDate = Object.values(latestSidByCity).sort((a,b)=>b.ts-a.ts)[0];
    latestSession = mergedItems.length ? { id: "all-cities", date: anyDate?.date, period: anyDate?.period, items: mergedItems } : undefined;
    sessionList = latestSession ? [latestSession] : [];
    firstSession = latestSession;
    hasHistory = false; // динамику между городами не показываем
  } else {
    const sessions = {};
    catData.forEach(item => {
    ...
    sessionList = Object.values(sessions).sort((a,b) => new Date(b.date) - new Date(a.date));
    latestSession = sessionList[0];
    firstSession = sessionList[sessionList.length - 1];
    hasHistory = sessionList.length > 1;
  }
  ...
  const AssessmentBlock = ({ session, city }) => {
    ...
    const cityKey = city || "Все";

    const runAssessment = async (force = false) => {
      setLoading(true); setError("");
      try {
        // 1. Проверяем кеш (если не форсим). Кеш раздельный по городу.
        if (!force) {
          const cached = await sb.getAssessment(filter, cityKey);
        ...
        const cityLabel = cityKey === "Все" ? "все города (Караганда + Астана суммарно)" : `город ${cityKey}`;
...
Верни ТОЛЬКО валидный JSON объект без markdown:
{
  ...
  "summary": "2-3 предложения общей оценки ассортимента с точки зрения трендов и современности — БЕЗ упоминания продаж, маржи, оборота или качества мастер-данных",
  ...
  "note": "опционально — предупреждение если данных мало"
        ...
        setData(parsed);
        // 3. Сохраняем в кеш (с привязкой к городу)
        await sb.saveAssessment(filter, cityKey, sessionId, trendsCount, parsed);
      } catch(e) {
    ...
    useEffect(() => { runAssessment(false); /* eslint-disable-next-line */ }, [filter, cityKey, sessionId, trendsCount]);
    ...
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(15,23,42,0.55)",zIndex:9999,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"24px 16px",overflowY:"auto"}}
      ...
      <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:16,width:"100%",maxWidth:920,boxShadow:"0 24px 64px rgba(15,23,42,0.14)",overflow:"hidden",marginBottom:24}}>
        ...
        <div style={{padding:"16px 20px",background:"#fffbeb",borderBottom:"1px solid #fde68a",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            <div style={{fontSize:11,color:"#92400e",fontWeight:700,marginBottom:2}}>📦 {isAll ? "АНАЛИЗ ДЕЙСТВУЮЩЕГО АССОРТИМЕНТА АЯН" : `АССОРТИМЕНТ · ${filter}${hasCities && cityFilter!=="Все" ? " · "+cityFilter : ""}`}</div>
          ...
          <div style={{padding:20}}>
            ...
            {hasCities && (
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,flexWrap:"wrap"}}>
                <span style={{fontSize:11,fontWeight:700,color:"#64748b",textTransform:"uppercase"}}>🏙 Город:</span>
                {["Все", ...citiesAvailable].map(c => {
                  const active = cityFilter === c;
                  const label = c === "Все" ? "Все города" : c;
                  return (
                    <button key={c} onClick={()=>setCityFilter(c)}
                      style={{background:active?"#7c3aed":"#fff",color:active?"#fff":"#64748b",border:"1px solid "+(active?"#7c3aed":"#e2e8f0"),borderRadius:7,padding:"5px 14px",fontSize:12,fontWeight:600,cursor:"pointer"}}>
                      {label}
                    </button>
                  );
                })}
            ...
            <AssessmentBlock session={latestSession} city={cityFilter} />
          </div>
        )}
...
export default function App() {
  ...
  const CAT_CONTEXTS = {
    ...
    "Free From / Без...": "Глобальный тренд Free From — продукты без определённых ингредиентов. Тренды 2026: безлактозное молоко и йогурты премиум (Valio EILA, Arla LactoFREE), растительное молоко (Oatly Barista, Alpro Oat, Minor Figures), растительные йогурты (Alpro, Oatly), без сахара (Rebel Kitchen, Natures Charm), безглютеновые альтернативы. Фокус: люди с непереносимостью лактозы/глютена + осознанное потребление. Слабые ниши в КЗ: растительное молоко, безлактозные йогурты премиум. НЕ дублировать Высокобелковые и Здоровое питание."
  };
...
[{"i":0,"product_type":"тип на русском"},{"i":1,"product_type":"..."}]`);
  ...
  const parseAssortmentFile = async (file) => {
    ...
    try {
      ...
      // Нормализация названия города (из имени листа или row[28])
      const KNOWN_CITIES = ["Караганда","Астана","Темиртау"];
      const normCity = (raw) => {
        if (!raw) return null;
        const s = String(raw).trim().toLowerCase();
        const hit = KNOWN_CITIES.find(c => c.toLowerCase() === s || s.includes(c.toLowerCase()));
        return hit || (String(raw).trim() ? String(raw).trim() : null);
      };

      // Парсим КАЖДЫЙ лист отдельно — каждый лист обычно = город
      const perCity = {}; // city -> { items: [], period: null }
      let totalItems = 0;
      ...
      for (const sheetName of wb.SheetNames) {
        ...
        let cityFromRow = null;
        const sheetItems = [];

        for (const row of rows) {
          if (!periodFrom && row[1] && row[10] && row[1] instanceof Date) { periodFrom = row[1]; periodTo = row[10]; }
          if (!periodFrom && row[1] && typeof row[1] === "string" && row[1].match(/\d{2}\.\d{2}\.\d{4}/)) { periodFrom = row[1]; periodTo = row[10]; }
          // Город из структурной единицы (row[28] в строке "Структурная единица")
          if (row[17] === "В группе из списка" && row[28] && typeof row[28] === "string") {
            const maybeCity = normCity(row[28]);
            if (maybeCity && KNOWN_CITIES.includes(maybeCity)) cityFromRow = maybeCity;
          }
          const name = row[1], qty = row[16], rev = row[26], cost = row[33];
          if (name && typeof name === "string" && name.length > 5 && typeof qty === "number" && qty > 0) {
            const margin = rev && cost && rev > 0 ? Math.round((rev - cost) / rev * 1000) / 10 : null;
            const avgPrice = qty > 0 ? Math.round((rev || 0) / qty) : null;
            sheetItems.push({ name: name.trim(), qty: Math.round(qty), revenue: rev || 0, margin, avg_price: avgPrice });
          }
        }
        if (sheetItems.length === 0) continue;

        // Город: приоритет — имя листа, фолбэк — row[28]
        const city = normCity(sheetName) || cityFromRow || "Не указан";
        const period = periodFrom ? `${formatDate(periodFrom)} — ${formatDate(periodTo) || "н/д"}` : null;

        if (!perCity[city]) perCity[city] = { items: [], period };
        sheetItems.forEach(it => { it.period = period; perCity[city].items.push(it); });
        totalItems += sheetItems.length;
      }

      if (totalItems === 0) throw new Error("Не удалось найти данные в файле");

      // Категория — всегда из текущего фильтра (кнопка доступна только когда filter !== "Все")
      const cat = filter !== "Все" ? filter : "Неизвестно";

      // Сохраняем по каждому городу отдельной сессией.
      // Перед вставкой удаляем прошлую загрузку ЭТОГО города в ЭТОЙ категории,
      // чтобы повторная загрузка обновляла город, а не плодила дубли.
      const cities = Object.keys(perCity);
      const sessionDate = new Date().toISOString();
      const summary = [];

      for (const city of cities) {
        const { items, period } = perCity[city];
        // Удаляем старые строки этого города+категории
        await fetch(`${SUPABASE_URL}/rest/v1/assortment?category=eq.${encodeURIComponent(cat)}&city=eq.${encodeURIComponent(city)}`, {
          method: "DELETE",
          headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` }
        });
        const sessionId = crypto.randomUUID();
        const rows = items.map(item => ({
          ...item, category: cat, city,
        ...
        summary.push(`${city}: ${items.length} SKU`);
      }

      await loadAssortmentFromSupabase();
      setAssortmentModal(true);
      alert(`✅ Загружено по городам:\n${summary.join("\n")}\n\nКатегория: ${cat}`);
    } catch(e) {
      ...
      try {
        ...
        const compResp = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            ...
            tools: [{ type: "web_search_20250305", name: "web_search" }],
            messages: [{ role: "user", content:
              `Проверь наличие товара "${item.name}" (бренд: ${brand}) у казахстанских ретейлеров. Выполни 3 отдельных поиска:

1. Поиск на Arbuz.kz: site:arbuz.kz "${brand}" — есть ли на сайте arbuz.kz?
2. Поиск Magnum Казахстан: "${brand}" magnum.kz OR kaspi.kz Magnum — есть ли в Magnum?
3. Поиск на Корзина: site:korzinavdom.kz "${brand}" — есть ли на korzinavdom.kz?

ОБЯЗАТЕЛЬНО укажи результат по каждому из трёх ретейлеров, даже если не нашёл — пиши "не найдено".

Формат ответа (строго):
Arbuz.kz: [ЕСТЬ/НЕТ] | цена: [цена или —] | SKU: [название или —]
Magnum: [ЕСТЬ/НЕТ] | цена: [цена или —] | SKU: [название или —]
Корзина (korzinavdom.kz): [ЕСТЬ/НЕТ] | цена: [цена или —] | SKU: [название или —]`
            }]
          })
        });
